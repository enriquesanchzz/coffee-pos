...
| 2 | Cadena de suministro | Compras: proveedores, órdenes de compra, recepción de mercancía, transferencias entre sucursales, conteos físicos. | 🟡 En progreso — **Proveedores + Órdenes de compra** construidos y verificados (`/compras`, ver `docs/CONTINUE.md`). Transferencias (`TransferManifest`) y conteos físicos (`PhysicalCount`) pendientes — modelos ya existen en el schema. |
...
## Fase 2 — en progreso (rama `feature/modulo-compras`)

**Proveedores + Órdenes de compra** construidos y verificados end-to-end
contra Postgres real (detalle en `docs/CONTINUE.md`): alta de proveedor con
costo cotizado por ingrediente, crear una orden, recibirla (total o
parcial) — genera `IngredientBatch`, sube `InventoryStock`, y si el costo
real difiere del cotizado actualiza `IngredientSupplier.cost` dejando
rastro en `IngredientCostHistory`.

Pendiente dentro de Fase 2: **transferencias entre sucursales/bodega**
(`TransferManifest`/`TransferLine`) y **conteos físicos**
(`PhysicalCount`/`PhysicalCountLine`) — modelos ya existen en el schema,
sin UI todavía.
